...
<%--                            <li><a href="#">Dashboard</a></li>--%>
                            <li class="active">Số lượng task: ${task.getTasksOfJob(job.getId())}</li>
                        </ol>
                    </div>
                    <!-- /.col-lg-12 -->
                </div>
                                ...
                                <div class="col-in row">
                                    <div class="col-md-6 col-sm-6 col-xs-6">
                                        ...
                                        <c:choose>
                                                ...
                                                <c:when test="${status.id eq 2}">
                                                    style="color: #ffae0d; font-weight: bold;"
                                                </c:when>
                                                <c:when test="${status.id eq 3}">
                                                    style="color: green; font-weight: bold;"
                                                </c:when>
                                        <c:otherwise>
                                            style="color: black; font-weight: bold;" <!-- Màu mặc định nếu không phù hợp với bất kỳ giá trị nào -->
                                        </c:otherwise>
                                        </c:choose>
                                        >${status.name}</h5>
                                    </div>
                                    <div class="col-md-6 col-sm-6 col-xs-6">
                                        <h3 class="counter text-right m-t-15 text-danger">${task.getTaskPercentageByJobAndStatusId(job.getId(), status.getId())}%</h3>
                                    </div>
                                    <div class="col-md-12 col-sm-12 col-xs-12">
                                        <div class="progress">
                                            <div class="progress-bar progress-bar-danger" role="progressbar"
                                                 aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width: ${task.getTaskPercentageByJobAndStatusId(job.getId(), status.getId())}%">
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
<%--                    <!-- /.col -->--%>
                ...
                <c:forEach items="${task.showTasksOfJob(job.getId())}" var="t">
                    <div class="row">
                        <div class="col-xs-12">
                            <a href="#" class="group-title">
                                <span style="color: #00AEEF">Tên task: ${t.getName()}</span>
                                <span style="
                                <c:choose>
                                <c:when test="${t.statusId eq 1}">
                                        color: #c12e2a;
                                </c:when>
                                <c:when test="${t.statusId eq 2}">
                                        color: #ffae0d;
                                </c:when>
                                <c:when test="${t.statusId eq 3}">
                                        color: green;
                                </c:when>
                                <c:otherwise>
                                        color: black; <!-- Màu mặc định nếu không phù hợp với bất kỳ giá trị nào -->
                                </c:otherwise>
                                </c:choose>
                                        ">Tình trạng: ${statuses.getStatusNameByItsId(t.getStatusId())}</span>
<%--                                <span>Tình trạng:${statuses.getStatusNameByItsId(t.getStatusId())}</span>--%>
                                <span style="color: #1a2942">Được thực hiện bởi:${user.getUserNameByUserId(t.getUserId())}</span>
